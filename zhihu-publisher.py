...
GITHUB_REPO_PREFIX = "https://raw.githubusercontent.com/tjuHaoXiaotian/Markdown4Zhihu/master/Data/"
...
def process_for_zhihu():
    ...
    with open(str(args.input), "r", encoding=chatest["encoding"]) as f:
        ...
        with open(str(args.input.parent) + "/" + (args.input.stem + "_for_zhihu.md"), "w+",
                  encoding=chatest["encoding"]) as fw:
            fw.write(lines)
        git_ops()
...
def rename_image_ref(m, original=True):
    ...
    if not (Path(image_folder_path.parent / m.group(1)).is_file() or Path(
        ...
        return m.group(0)
    if os.path.getsize(str(image_folder_path.parent / m.group(1 + int(original)))) > COMPRESS_THRESHOLD:
        if original:
